...
float ballA (float3 p) {
    return ball (p, 3.0*sin(0.8*_Time.y),
                    0.5,
                    0.0);
}
...
float ballD (float3 p) {
    return ball (p, 2.0*cos (0.3*_Time.y),
                    1.5*sin (2.0*_Time.y),
                   -0.2);
}
...
float3 colorfunc (float3 p) {
    const float3 colorA = float3(0.8,0.1,0.1);
    const float3 colorB = float3(0.8,0.1,0.1);
    const float3 colorC = float3(0.1,0.8,0.1);
    const float3 colorD = float3(0.2,0.2,1.0);
...
float4 runDemo(float2 fragCoord)
{
    ...
    if (hit) {
        ...
        float3 newDir = reflect (rayDir, normal);
